...
export default function ManageSellers() {
  ...
  useEffect(() => { load(); }, [load]);
  const decide = async (id, decision) => { try { await financialApi.decideSeller(id, decision); toast.success(`Seller ${decision}d`); load(); } catch (e) { toast.error(e.response?.data?.detail || 'Action failed'); } };
  ...
  return <div className="page-wrapper"><section className="dashboard-page"><div className="container">
    ...
    <div className="admin-card dashboard-table-card"><div className="table-scroll"><table className="data-table"><thead><tr><th>Seller</th><th>Business</th><th>Phone</th><th>Status</th><th>Actions</th></tr></thead><tbody>
      {rows.map((seller) => <tr key={seller.id}><td><strong>{seller.name}</strong><br/><small className="text-gray">{seller.email}</small></td><td>{seller.business_name}</td><td>{seller.phone}</td><td><span className={`dashboard-status status-${String(seller.approval_status).toLowerCase()}`}>{seller.approval_status}</span></td><td><div className="flex gap-2"><button className="btn btn-primary btn-sm" onClick={() => decide(seller.id, 'approve')} disabled={seller.approval_status === 'APPROVED'}>Approve</button><button className="btn btn-secondary btn-sm" onClick={() => decide(seller.id, 'reject')} disabled={seller.approval_status === 'REJECTED'}>Reject</button></div></td></tr>)}
