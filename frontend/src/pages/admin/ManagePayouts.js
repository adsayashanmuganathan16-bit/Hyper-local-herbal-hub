...
export default function ManagePayouts() {
  ...
  return <div className="page-wrapper"><section className="dashboard-page"><div className="container">
    ...
    <div className="admin-card dashboard-table-card table-scroll"><table className="data-table"><thead><tr><th>Created (Sri Lanka)</th><th>Seller</th><th>Gross</th><th>Commission</th><th>Net</th><th>Status</th><th>Actions</th></tr></thead><tbody>{rows.map((row) => <tr key={row.id}><td>{formatDateTime(row.created_at)}</td><td><strong>{row.business_name || row.seller_name || 'Unknown seller'}</strong><br/><small className="text-gray">#{String(row.seller_id || '').slice(0, 8).toUpperCase()}</small></td><td>{formatCurrency(row.gross_amount)}</td><td>{formatCurrency(row.commission_amount)}</td><td><strong>{formatCurrency(row.net_amount)}</strong></td><td><span className={`dashboard-status status-${String(row.status).toLowerCase().replaceAll('_', '-')}`}>{String(row.status).replaceAll('_', ' ')}</span></td><td><div className="flex gap-2"><button className="btn btn-secondary btn-sm" onClick={() => openReceipt(row.id)}><FiDownload /> Receipt</button>{['PENDING','READY_FOR_MANUAL_TRANSFER'].includes(row.status) ? <button className="btn btn-primary btn-sm" onClick={() => paid(row.id)}>Mark Paid</button> : row.status === 'FAILED' && row.retry_count < 3 ? <button className="btn btn-secondary btn-sm" onClick={() => retry(row.id)}>Retry</button> : null}</div></td></tr>)}{!rows.length && <tr><td className="dashboard-empty" colSpan="7">No payout records yet.</td></tr>}</tbody></table></div>
  </div></section></div>;
}
