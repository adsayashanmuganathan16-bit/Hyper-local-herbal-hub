...
export default function SellerEarnings() {
  ...
  return <div className="page-wrapper"><section className="dashboard-page"><div className="container">
    ...
    {profile?.bank_account && <div className="admin-card bank-summary"><div><h2>Your payout account</h2><p><strong>{profile.bank_account.bank_name}</strong> · {profile.bank_account.branch}</p><p>{profile.bank_account.account_holder_name} · {profile.bank_account.account_number}</p></div><span className={`dashboard-status ${profile.verification_status === 'VERIFIED' ? 'status-approved' : 'status-pending'}`}>{profile.verification_status === 'VERIFIED' ? 'Verified' : String(profile.verification_status || 'PENDING').replaceAll('_', ' ')}</span></div>}
    {profile?.admin_bank_account && <div className="admin-card bank-summary mt-4"><div><h2>Platform admin account</h2><p><strong>{profile.admin_bank_account.bank_name}</strong> · {profile.admin_bank_account.branch}</p><p>{profile.admin_bank_account.account_holder_name} · {profile.admin_bank_account.account_number}</p></div><span className="dashboard-status status-approved">Platform account</span></div>}
